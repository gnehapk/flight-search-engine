...
const createSession = async (params) => {
  console.log('Creating a session..', formatParams(params));
  try {
...
const poll = async (location, params) => {
  await throttle();
  console.log('Polling results..');
  try {
    const { stops, pageIndex, pageSize } = params;
    const response = await fetch(`${location}?apikey=${config.apiKey}&stops=${stops}&pageIndex=${pageIndex}
    &pageSize=${pageSize}`);
...
const getResults = async (location, params) => {
  try {
    const response = await poll(location, params);
    if (response.Status && response.Status === 'UpdatesComplete') {
      return response;
    }
    return await getResults(location, params);
  } catch (err) {
    throw err;
  }
};
...
const search = async (params) => {
  try {
    ...
    return await getResults(locationToPoll, params);
  } catch (err) {
    throw err;
  }
};
